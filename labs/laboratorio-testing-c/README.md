...
> R: Porque inicializa la cantidad en 0, de no hacerse tendríamos incializada la variable en algo que no sabemos
...
> R: 24, esperado 700, obtenido 350

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería multiplicar la cantidad por el precio
...
> R: La línea 29 y 30 que corresponden a la funcion carrito_descuento, no se ejecutan porque nunca se la usa.
...
> R: El unitario prueba una función aislada; el de integración prueba varias funciones combinadas. El bug de carrito_total lo detectó primero un test unitario (test_total_con_cantidad).
...
> R: En C no hay chequeo automático de límites de array: escribir fuera de items[MAX_ITEMS] pisa memoria ajena sin avisar (comportamiento indefinido). Python y Java lanzan una excepción (IndexOutOfBounds) y frenan la ejecución antes de corromper memoria.
...
> R: Escribiendo tests validás el comportamiento real ejecutando el código con casos concretos, en vez de "creer" que está bien con solo leerlo; los tests detectan errores que a simple vista pasan desapercibidos (como en P14).
...
> R: Con cantidad = 1, el bug (que no multiplicaba por cantidad) daba el mismo resultado que el correcto (350 = 350×1), por eso el test no lo detectó; con cantidad = 2 la diferencia (700 vs 350) sí quedó expuesta.
